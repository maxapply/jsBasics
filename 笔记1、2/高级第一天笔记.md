...
​				触发器：$(元素).trigger('click')

​				触发器：$(元素).triggerHandler（'click'）
...
// 继承
class 类 extends 类名 {super}

var obj = new 类名('名字', 22);

三个注意点：
	必须先定义类，再实例化对象
    在类里面调用属性和方法的时候，this.属性，this.方法()
	this指向：构造函数里面的this指向实例，方法里面的this指向调用者
```
...
注意：先定义类再实例化，再类里面调用属性或者方法：this.属性或this.方法()

this指向：构造函数红this指向实例对象，方法里面的this指向调用者
